<template>
    <div ref="containerEl" :class="styles.entityContainer">
        <ErDiagramEntity v-bind="item" v-for="item in items" />
        <slot />
    </div>
</template>
...
import { updateConnections, autoCorrectPositions, updateEntityPosition } from './ErDiagramConfig';
...
onMounted(() => setTimeout(() => {
    ...
    for (const entityElement of entityElements) {
        ...
        const entity = {
            ...
            columns: columns,
            el: entityElement, pointerEl: pointerElement,
            x: 0, y: 0,
            w: entityElement.offsetWidth, h: entityElement.offsetHeight
        } as EntityElement
